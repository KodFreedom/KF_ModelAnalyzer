...
void UnityChanWaitMotionState::ChangeMotion(Animator& animator)
{
	...
	{
		animator.Change(new BlendMotionState(current_motion_name_, new UnityChanRunMotionState(0), current_motion_frame_counter_, 10);
		return;
	}
	if(animator.GetIsJump() == true)
	{
		animator.Change(new BlendMotionState(current_motion_name_, new UnityChanJumpMotionState(0), current_motion_frame_counter_, 10);
		return;
	}
	...
	{
		animator.Change(new BlendMotionState(current_motion_name_, new UnityChanFallDownMotionState(0), current_motion_frame_counter_, 10);
		return;
	}
}